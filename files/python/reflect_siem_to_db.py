...
def mark_as_false_positive(SETTINGS, execution_id):
    ...
    for ioc_type in type_list:
        ...
        try:
            total_by_type_in_db = sqlite_command(SETTINGS, SQL_TO_LIST_ALL_IOCS_NOT_FALSE_POSITIVE, (ioc_type, type_ttl))
            ...
            if SETTINGS['SIEM_SETTINGS']['SIEM'] == 'QRADAR':
                total_by_type_in_siem = qradar_list_data(SETTINGS, generate_siem_storage_name(SETTINGS, ioc_type))

                for ioc in total_by_type_in_siem['data']:
                    siem_list.append(str(ioc['value']).strip())

            elif SETTINGS['SIEM_SETTINGS']['SIEM'] == 'SPLUNK':
                siem_list = splunk_list_data(SETTINGS, generate_siem_storage_name(SETTINGS, ioc_type))

            elif SETTINGS['SIEM_SETTINGS']['SIEM'] == 'CSV':
                siem_list = csv_list_data(SETTINGS, generate_siem_storage_name(SETTINGS, ioc_type))
            ...
            if len(siem_list) < len(ioc_list):
                ...
                for item in ioc_list:
                    ...
                    progress = progress_logging(SETTINGS, len(ioc_list), count_progress)
                    
                    if progress is not None:
                        message = 'mode="AGENT" details="Checking {} IOCs from {} in DB" value="{} ({}/{})"'.format(ioc_type, SETTINGS['SIEM_SETTINGS']['SIEM'], progress, count_progress, len(ioc_list))
                        logging.info(message)
                    ...
                    if item not in siem_list:
                        ...
                        sqlite_command(SETTINGS, SQL_TO_UPDATE_IOC_AS_FALSE_POSITIVE.format(comparator),
                ...
                for item in siem_list:
                    ...
                    if progress is not None:
                        message = 'details="Checking {} IOCs from DB in {}" value="{} ({}/{})"'.format(ioc_type, SETTINGS['SIEM_SETTINGS']['SIEM'], progress, count_progress, len(ioc_list))
                        logging.info(message)
                    ...
                    if item not in ioc_list:
                        ...
                        if SETTINGS['SIEM_SETTINGS']['SIEM'] == 'QRADAR':
                            ...
                            post_status, post_text = csv_delete_item(SETTINGS, siem_storage)

                        if SETTINGS['SIEM_SETTINGS']['SIEM'] == 'CSV':
                            count += 1
                            counter_limit += 1
                            message = 'details="Removed the IOC from database because was removed from {} ' \
                                      'by another source" count="{}" ' \
                                      'type="{}" value="{}"'.format(SETTINGS['SIEM_SETTINGS']['SIEM'], count, siem_storage,
                                                                    item.encode('ascii', 'ignore').decode('utf-8'))
                            logging.info(message)
                            ...
                            message = 'details="Removed the IOC from database because was removed from {} ' \
                                      'by another source" count="{}" ' \
                                      'type="{}" value="{}"'.format(SETTINGS['SIEM_SETTINGS']['SIEM'], count, ioc_type,
                                                                    item.encode('ascii', 'ignore').decode('utf-8'))
                            logging.info(message)
                        else:
                            message = 'details="Error while try to remove the IOC from database because ' \
                                      'was removed from {} - ' \
                                      '{}, {}" count="{}" type="{}" value="{}"'.format(SETTINGS['SIEM_SETTINGS']['SIEM'],
