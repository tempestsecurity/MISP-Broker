...
def qradar_create_refence_set(all_settings, element_type, reference_set_name, ttl):
    ...
    if ttl == 0 or ttl == 360000:
        url = '{}element_type={}&name={}&' \
              'timeout_type=LAST_SEEN'.format(all_settings['SIEM_SETTINGS']['QRADAR_REFERENCE_DATA_URL'], element_type, reference_set_name)

    else:
        url = '{}element_type={}&name={}&' \
              'timeout_type=LAST_SEEN&time_to_live={}'.format(all_settings['SIEM_SETTINGS']['QRADAR_REFERENCE_DATA_URL'], element_type, reference_set_name, generate_qradar_ttl(ttl))

    post_data = requests.post(url, headers=all_settings['SIEM_SETTINGS']['QRADAR_HEADERS'], verify=all_settings['SIEM_SETTINGS']['SIEM_VERIFY_SSL'])

    return post_data.status_code, post_data.text


def qradar_load(all_settings, reference_set_name, data_list):
    ...
    for splitter_list in split_data_list:
        ...
        url = '{}{}?&namespace=SHARED'.format(all_settings['SIEM_SETTINGS']['QRADAR_BULK_URL'], reference_set_name)
        post_data = requests.post(url, data=dumps(splitter_list), headers=all_settings['SIEM_SETTINGS']['QRADAR_HEADERS'], verify=all_settings['SIEM_SETTINGS']['SIEM_VERIFY_SSL'])
...
def qradar_list_data(all_settings, reference_set_name):

    url = '{}{}'.format(all_settings['SIEM_SETTINGS']['QRADAR_REFERENCE_DATA_LIST_ITEM_URL'], reference_set_name)
    json_data = requests.get(url, headers=all_settings['SIEM_SETTINGS']['QRADAR_HEADERS'], verify=all_settings['SIEM_SETTINGS']['SIEM_VERIFY_SSL']).json()

    return json_data


def qradar_purge_reference_set(all_settings, reference_set_name):

    url = '{}{}?purge_only=true'.format(all_settings['SIEM_SETTINGS']['QRADAR_SIEM_STORAGE_URL'], reference_set_name)
    post_data = requests.delete(url, headers=all_settings['SIEM_SETTINGS']['QRADAR_HEADERS'], verify=all_settings['SIEM_SETTINGS']['SIEM_VERIFY_SSL'])
...
def qradar_delete_item(all_settings, reference_set_name, item):
    ...
    try:
        delete_data = requests.delete(url, headers=all_settings['SIEM_SETTINGS']['QRADAR_HEADERS'], verify=all_settings['SIEM_SETTINGS']['SIEM_VERIFY_SSL'])
...
def qradar_add_to_reference_set(all_settings, ioc_type, element_type, days, update_time, data_list, execution_id):
    reference_set = generate_siem_storage_name(all_settings, ioc_type)
    ...
    if not check_siem_storage_in_db(all_settings, ioc_type):
        ...
        post_status, post_text = qradar_create_refence_set(all_settings, element_type, reference_set, days)
        ...
        if check_status_code(post_status) or already_in_use:
            sqlite_command(all_settings, SQL_TO_ADD_SIEM_STORAGE_IN_DB, (ioc_type, reference_set, 1, update_time))
        ...
        post_status, post_text = qradar_load(all_settings, reference_set, data_list)
