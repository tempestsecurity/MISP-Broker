...
def sqlite_command(SETTINGS, sql_query, tuple_list=()):
    try:
...
def execution_start_register(SETTINGS, mode):
    now = datetime.datetime.now()
    start_execution = now.strftime(DATE_TIME_FORMAT)

    sqlite_command(SETTINGS, SQL_TO_ADD_EXECUTION_HISTORY, (mode, start_execution))

    execution_id = sqlite_command(SETTINGS, SQL_TO_GET_ID_OF_EXECUTION_HISTORY, (mode, start_execution))[1][-1][0]

    message = str(sqlite_command(SETTINGS, SQL_TO_GET_DATA_OF_EXECUTION_HISTORY, (execution_id, )))
...
def execution_stop_register(SETTINGS, execution_id, status):

    start_execution = sqlite_command(SETTINGS, SQL_TO_GET_START_EXECUTION, (execution_id, ))[1][0][0]
    ...
    sqlite_command(SETTINGS, SQL_TO_UPDATE_EXECUTION_HISTORY, (now_str, total_time, status, execution_id))
...
def connection_error_register(SETTINGS, destination, details):
    now = datetime.datetime.now()
    now_str = now.strftime(DATE_TIME_FORMAT)
    sqlite_command(SETTINGS, SQL_TO_ADD_CONNECTION_ERROR,
                   (destination, now_str, details))

    execution_id = sqlite_command(SETTINGS, SQL_TO_GET_CONNECTION_ERROR,
                                  (destination, now_str,
                                   details))[1][-1][0]

    message = str(sqlite_command(SETTINGS, SQL_TO_GET_DATA_CONNECTION_ERROR, (execution_id, )))
    message = re.sub('"', "'", message)
