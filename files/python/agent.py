...
def agent(SETTINGS, events_count_by_type):
    ...
    execution_id = execution_start_register(SETTINGS, mode)

    try:
        # Get from local database the amount of IOCs
        total_of_iocs_in_db = sqlite_command(SETTINGS, SQL_TO_LIST_ALL_IOCS)[1]
        ...
        reflect_db_to_siem(SETTINGS, execution_id)
        logging.debug("Ending: reflect_db_to_siem")
        logging.debug("Starting: mark_as_false_positive")
        mark_as_false_positive(SETTINGS, execution_id)
        ...
        for ioc_type_settings in type_list:
            ...
            body_by_type = SETTINGS['MISP_SETTINGS']['MISP_BODY_BY_TYPE']
            body_by_type['type']['AND'].clear()
            body_by_type['type']['AND'].append(ioc_type_settings)

            # Body used to get sightings
            body_sightings = SETTINGS['MISP_SETTINGS']['MISP_BODY_SIGHTINGS']

            # Body used to get last updates in MISP
            body_last_updates = SETTINGS['MISP_SETTINGS']['MISP_BODY_LAST_UPDATES']

            # Get this execution date and time
            now = datetime.datetime.now()
            timestamp_last_updates = now - datetime.timedelta(hours=SETTINGS['BROKER_SETTINGS']['UPDATE_LOOKBACK'])
            ...
            last_sync = sqlite_command(SETTINGS, SQL_TO_GET_LAST_SYNC, (ioc_type_settings,))
            ...
            if last_sync[1]:
                ...
                last_sync_date = '{} 00:00:00'.format(SETTINGS['BROKER_SETTINGS']['START_DATE'])

                sqlite_command(SETTINGS, SQL_TO_ADD_LAST_SYNC, (ioc_type_settings, last_sync_date))
            ...
            try:
                ...
                if days == 0:
                    days = (now - datetime.datetime.strptime(SETTINGS['BROKER_SETTINGS']['START_DATE'], DATE_FORMAT)).days
            ...
            if int(earliest.timestamp()) < int(last_sync_date.timestamp()):
                ...
                if total_hours <= SETTINGS['BROKER_SETTINGS']['RANGE_TIME']:
                    total_hours = SETTINGS['BROKER_SETTINGS']['RANGE_TIME']
            ...
            while hour_count <= total_hours:
                hour_count += SETTINGS['BROKER_SETTINGS']['RANGE_TIME']

                events_on_misp = {}

                latest = earliest + datetime.timedelta(hours=(SETTINGS['BROKER_SETTINGS']['RANGE_TIME'] - 1))
                ...
                while not misp_connection_status:
                    try:
                        ...
                        response = requests.post(SETTINGS['MISP_SETTINGS']['MISP_API_URL'], headers=SETTINGS['MISP_SETTINGS']['MISP_HEADERS'], json=body_by_type, verify=False)
                        ...
                        response = requests.post(SETTINGS['MISP_SETTINGS']['MISP_API_URL'], headers=SETTINGS['MISP_SETTINGS']['MISP_HEADERS'], json=body_last_updates,
                                                 verify=False)
                        query_json = response.json()
                        try:
                            ...
                            details = '{} details="Cannot get last updated attributes" ' \
                                      'value="{} {}"'.format(step_timestamp, SETTINGS['MISP_SETTINGS']['MISP_API_URL'], e)
                            logging.error(details)

                        response = requests.post(SETTINGS['MISP_SETTINGS']['SIGHTINGS_RECENT_URL'], headers=SETTINGS['MISP_SETTINGS']['MISP_HEADERS'], json=body_last_updates,
                                                 verify=False)
                        query_json = response.json()
                        ...
                        try:
                            ...
                            details = '{} details="Cannot get last updated sightings" ' \
                                      'value="{} {} {}"'.format(step_timestamp, SETTINGS['MISP_SETTINGS']['MISP_API_URL'], e, query_json)
                            logging.error(details)
                        ...
                        if value_list:
                            ...
                            response = requests.post(SETTINGS['MISP_SETTINGS']['MISP_API_URL'], headers=SETTINGS['MISP_SETTINGS']['MISP_HEADERS'], json=attributes_sightings_json,
                                                     verify=False)
                            query_json = response.json()
                        ...
                        details = '{} details="Cannot connect to MISP server" ' \
                                  'value="{} {}"'.format(step_timestamp, SETTINGS['MISP_SETTINGS']['MISP_API_URL'], e)
                        connection_error_register(SETTINGS, 'MISP', details)
                        time.sleep(SETTINGS['BROKER_SETTINGS']['WAIT_TIME'])

                    check_run_status(SETTINGS, execution_id)

                earliest = earliest + datetime.timedelta(hours=SETTINGS['BROKER_SETTINGS']['RANGE_TIME'])
                ...
                for iocs in split_data_list:
                    ...
                    for ioc in iocs:
                        ...
                        while not misp_connection_status:
                            if event not in events_on_misp:
                                try:
                                    ...
                                    response = requests.post('{}{}'.format(SETTINGS['MISP_SETTINGS']['SIGHTINGS_URL'], event), headers=SETTINGS['MISP_SETTINGS']['MISP_HEADERS'],
                                                             json=body_sightings, verify=False)
                                    query_json = response.json()
                        ...
                        is_a_exception, epoch = is_exception(SETTINGS, ioc, events_on_misp[event])
                        ...
                        search_ioc = []
                        search_ioc.clear()
                        search_ioc = sqlite_command(SETTINGS, SQL_TO_GET_IOC_BY_MD5, (md5,))[1]
                        ...
                        if len(search_ioc) == 0:
                            already_exists = sqlite_command(SETTINGS, SQL_TO_GET_IOC_BEFORE_DELETE, (ioc_type, value))
                            already_exists = len(already_exists[1])

                            sqlite_command(SETTINGS, SQL_TO_DELETE_IOC_BEFORE_EXCEPTION, (ioc_type, value))
                            success_count += 1

                            if is_a_exception and already_exists:
                                insert_status = sqlite_command(SETTINGS, SQL_TO_ADD_IOC_EXCEPTION, (ioc_type, value,
                                                                                          last_sync_timestamp,
                                ...
                                insert_status = sqlite_command(SETTINGS, SQL_TO_ADD_IOC, (ioc_type, value, last_sync_timestamp,
                                                                                attribute_timestamp, md5,
                                                                                lived_days))[0]
                        ...
                        # sqlite_command(SETTINGS, SQL_TO_UPDATE_LAST_SYNC, (body_by_type['to'], ioc_type))
                        sqlite_command(SETTINGS, SQL_TO_UPDATE_LAST_SYNC, (body_by_type['to'], ioc_type_settings))

                        progress = progress_logging(SETTINGS, len(iocs_from_misp), ioc_count)
                    ...
                    if ioc_update == 0 and ioc_count > 0:
                        ...
                        sqlite_command(SETTINGS, SQL_TO_UPDATE_LAST_SYNC, (body_by_type['from'], ioc_type_settings))
            ...
            sqlite_command(SETTINGS, SQL_TO_UPDATE_LAST_SYNC, (datetime.datetime.now().strftime(DATE_TIME_FORMAT),
                                                     ioc_type_settings))
        ...
        update_lived_days(SETTINGS, mode)

        final_status = '{}\n{}'.format(final_status, message)
        logging.info(message)

        execution_stop_register(SETTINGS, execution_id, final_status.strip())
        ...
        execution_stop_register(SETTINGS, execution_id, a)
        return events_count_by_type
