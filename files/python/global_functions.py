...
import socket
import pathlib
from files.python.constants import TYPE_LIST, TYPE_LIST_HELP, TYPE_LIST_SAMPLE, REGEX_IPV4, DATE_FORMAT, SQL_TO_CHECK_SIEM_STORAGE_IN_DB
from files.python.sqlite_functions import execution_stop_register, sqlite_command


def check_run_status(all_settings, execution_id=None):

    if not pathlib.Path(all_settings['RUNNING_FILE']).exists():
        message = 'details="{} not found, stopping service now"'.format(all_settings['RUNNING_FILE'])
...
def get_type_and_feed_lists(all_settings, execution_id):
    ...
    try:
        if all_settings['MISP_SETTINGS']['MISP_TYPE_LIST']:
            tl_file = 'type_list-{}.txt'.format(all_settings['MISP_SETTINGS']['MISP_TYPE_LIST'])
    except Exception:
        pass
...
def progress_logging(all_settings, total, actual):
    total = int(total)
    start_percent = 0
    progress_count = 1
    percent_step = 100 / all_settings['BROKER_SETTINGS']['PERCENT_STEP_LOG']
    ...
    while start_percent <= 100:
        ...
        start_percent += all_settings['BROKER_SETTINGS']['PERCENT_STEP_LOG']
        progress_count += step
    ...
    if actual in logging_list:
        ...
        if actual == total:
            ...
            actual_percent = actual_percent * all_settings['BROKER_SETTINGS']['PERCENT_STEP_LOG']
...
def is_exception(all_settings, ioc, sightings_list):
    comment = False
    if all_settings['BROKER_SETTINGS']['MISP_COMMENT_EXCEPTION'] != '':
        comment = re.findall(all_settings['BROKER_SETTINGS']['MISP_COMMENT_EXCEPTION'].lower(), str(ioc['comment']).lower())

    tags = False
    if all_settings['BROKER_SETTINGS']['MISP_TAG_EXCEPTION'] != '':
        try:
            for tag in ioc['Tag']:
                if re.findall(all_settings['BROKER_SETTINGS']['MISP_TAG_EXCEPTION'], str(tag['name']).lower()):
                    tags = True
        except Exception:
            pass
...
def check_siem_storage_in_db(all_settings, ioc_type):
    last_sync = sqlite_command(all_settings, SQL_TO_CHECK_SIEM_STORAGE_IN_DB, (ioc_type,))
...
def generate_siem_storage_name(all_settings, ioc_type=None):
    if ioc_type is not None:
        if all_settings['SIEM_SETTINGS']['SIEM'].upper() == 'SPLUNK' or all_settings['SIEM_SETTINGS']['SIEM'].upper() == 'CSV':
            siem_storage_name = str(ioc_type.replace('|', '_'))
        else:
